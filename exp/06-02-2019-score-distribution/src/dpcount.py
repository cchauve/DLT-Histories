...
def randGen(u,state="H",n=0,S=[],H=[],D=[],T=[],ALLOW_TRANSFERS=False):
    ...
    if state == "H":
        if u.isLeaf():
            if n==1:
                ...
                rand -= D[i][n]
                if rand<0:
                    return randGen(u,'D',n,S,H,D,T,ALLOW_TRANSFERS)
                rand -= T[i][n]
                if rand<0:
                    return randGen(u,'T',n,S,H,D,T,ALLOW_TRANSFERS)
        else:
            rand = random.random()*H[i][n]
            rand -= D[i][n]
            if rand<0:
            ...
            rand -= S[i][n]
            if rand<0:
                return randGen(u,'S',n,S,H,D,T,ALLOW_TRANSFERS)
            rand -= T[i][n]
            if rand<0:
                return randGen(u,'T',n,S,H,D,T,ALLOW_TRANSFERS)
    ...
    if state == "D":
        ...
        for m in range(1,n):
            ...
            if rand<0:
                return [DupLbl(u)]+randGen(u,'H',n-m,S,H,D,T,ALLOW_TRANSFERS)+randGen(u,'H',m,S,H,D,T,ALLOW_TRANSFERS)
    ...
    if state == "T" and ALLOW_TRANSFERS:
        ...
        for v in receivers:
            ...
            for m in range(1,n):
                ...
                if rand<0:
                    return [TranLbl(u,v)]+randGen(u,'H',n-m,S,H,D,T,ALLOW_TRANSFERS)+randGen(v,'H',m,S,H,D,T,ALLOW_TRANSFERS)
    ...
    if state == "S" and (not u.isLeaf()):
        if u.isBinary():
            ...
            rand -= Loss(l)*H[rid][n]
            if rand<0:
                return [LossLbl(l)] + randGen(r,'H',n,S,H,D,T,ALLOW_TRANSFERS)
            rand -= Loss(r)*H[lid][n]
            if rand<0:
                return randGen(l,'H',n,S,H,D,T,ALLOW_TRANSFERS) + [LossLbl(r)]
            for m in range(1,n):
                rand -= H[lid][n-m]*H[rid][m]
                if rand<0:
                    return randGen(l,'H',n-m,S,H,D,T,ALLOW_TRANSFERS) + randGen(r,'H',m,S,H,D,T,ALLOW_TRANSFERS)
        if u.isUnary():
            c = u.getChild()
            cid = c.getID()
            rand = random.random()*S[i][n]
            rand -= H[cid][n]
            if rand<0:
                return randGen(c,'H',n,S,H,D,T,ALLOW_TRANSFERS)
