...
def fit2Distribution(model,tree,nbGenes,target,gamma,nbSamples,maxIterations,epsilon,X,trace):
    nbIterations = 1
    dist2Target  = epsilon+1.0
    weights      = {z:1.0 for z in target.keys()}
    while nbIterations<=maxIterations and dist2Target>epsilon:
        S,H,D,T = dpcount.fillMatrices(tree,nbGenes,model,X,weights)
        frequencies = {z:0.0 for z in target.keys()}    
        for j in range(0,nbSamples):
            history = dpcount.randGen(tree,'H',nbGenes,S,H,D,T,model,X)
            for z in target.keys():
                frequencies[z] += history.count('Z'+str(z))/float(nbSamples)
        difference  = [target[z]-frequencies[z] for z in target.keys()]
        dist2Target = npla.norm(np.array(difference),np.inf)
        for z in target.keys():
            weights[z] = weights[z]*math.pow(gamma,target[z]-frequencies[z])    
        if trace:
            print('Iteration '+str(nbIterations)+' distance '+str(dist2Target))
        nbIterations+=1
    return((dist2Target,weights))

def rejectHistory(history,target,trace):
    reject = False
    count  = {z:0 for z in target.keys()}
    for z in target.keys():
        count[z] = history.count('Z'+str(z))
        if target[z]!=count[z]:
            reject=True        
    return((reject,count))
            
if __name__ == "__main__":
    k         = int(sys.argv[1]) # Species tree size
    maxCopies = int(sys.argv[2]) # Max number of copies per species
    maxIter   = int(sys.argv[3]) # Number of iterations of updating WZ
    nbSamples = int(sys.argv[4]) # Number of samples per iteration
    epsilon   = float(sys.argv[5]) # Precision of the distribution
    if sys.argv[6]=='true':
        trace=True
    else:
        trace=False
    ...
    for u in t.allNodes():
        if u.isLeaf():
            TARGET[u.getID()] = random.randint(1,maxCopies)
            nbGenes += TARGET[u.getID()]

    if trace:
        strInput  = "#0 Species tree "+st
        strTarget = "Target    \t"
        for z in TARGET.keys():
            strTarget+="Z"+str(z)+":"+str('%.2f'%TARGET[z])+" "
        print(strInput)
        print(strTarget)

    (dist,weights) = fit2Distribution(model,t,nbGenes,TARGET,GAMMA,nbSamples,maxIter,epsilon,X,trace)          
    S,H,D,T     = dpcount.fillMatrices(t,nbGenes,model,X,weights)
    nbRejected  = 0
    frequencies = {z:0.0 for z in TARGET.keys()}    
    for j in range(0,nbSamples):
        history = dpcount.randGen(t,'H',nbGenes,S,H,D,T,model,X)
        (reject,count) = rejectHistory(history,TARGET,trace)
        for z in TARGET.keys():
            frequencies[z]+=count[z]/float(nbSamples)
        if reject:
            nbRejected+=1            
            #if trace:
            #    strHistory = "History    \t"
            #    for z in TARGET.keys():
            #        strHistory+="Z"+str(z)+":"+str('%.2f'%count[z])+" "
            #    print(strTarget)
            #    print(strHistory)

    print('Number of rejected histories '+str(nbRejected))
    if trace:
        strSample = "Frequencies\t"
        for z in TARGET.keys():
            strSample+="Z"+str(z)+":"+str('%.2f'%frequencies[z])+" "
        print(strTarget)
        print(strSample)
