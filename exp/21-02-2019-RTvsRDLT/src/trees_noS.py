...
if __name__ == "__main__":
    ...
    model1 = {'D':True,'L':True,'T':True}
    model2 = {'D':False,'L':True,'T':True}
    model3 = {'D':True,'L':True,'T':False}
        
    print("#n\tRDLT\tRLT\tRDL\tRLT/RDLT\tRDL/RDLT\tRLT/RDL")
    for j in range(0,nbt):
        t = randomBinaryTree(k)
        labelTree(t)
        st = t.asNewick()
        (rk,rkt) = rankTreeRandomly(t)
        srk = printRanking(rk)
        srkt = rkt.asNewick()        
        print("# Species tree "+st+" ranking "+str(srk)+" ranked species tree "+srkt)

        S1,H1,D1,T1 = dpcount_noS.fillMatrices(rkt,nMax,model1,1.0)
        nbHistories1 = H1[rkt.getID()][nMax]
        S2,H2,D2,T2 = dpcount_noS.fillMatrices(rkt,nMax,model2,1.0)
        nbHistories2 = H2[rkt.getID()][nMax]
        S3,H3,D3,T3 = dpcount_noS.fillMatrices(rkt,nMax,model3,1.0)
        nbHistories3 = H3[rkt.getID()][nMax]
        print(str(nMax)+"\t"+str(nbHistories1)+"\t"+str(nbHistories2)+"\t"+str(nbHistories3)+"\t"+str(nbHistories2/nbHistories1)+"\t"+str(nbHistories3/nbHistories1)+"\t"+str(nbHistories2/nbHistories3))

        if sample==1:
            for i in range(1,1000):
                h2 = dpcount_noS.randGen(rkt,'H',2,S2,H2,D2,T2,model1)
                print("RDLT\t"+str(h2))
            #for i in range(1,1000):
            #    h2 = dpcount_noS.randGen(rkt,'H',2,S2,H2,D2,T2,model2)
            #    print("RLT\t"+str(h2))
            #for i in range(1,1000):
            #    h3 = dpcount_noS.randGen(rkt,'H',2,S3,H3,D3,T3,model3)
            #    print("RDL\t"+str(h3))
