...
def isBalanced(t):
    ...
    if len(children)<=1:
        ...
        vl = t.getLeft()
        vr = t.getRight()
        return (abs(vl.getSize()-vr.getSize())<=1) and isBalanced(vl) and isBalanced(vr)
...
# Compute the Horton-Strahler number of a tree
def updateHS(x,y):
    if x==y:
        return(x+1)
    else:
...
def computeHortonStrahler(t):
    allNodes = t.allNodes()
    HS = {u.getID(): 1 for u in allNodes}
    nodesQueue = []
    for u in allNodes:
        if u.isLeaf():
            nodesQueue.append(u)
    while len(nodesQueue)>0:
        ...
        if not u.isRoot():
            v = u.getParent()
            if v.isUnary():
                HS[v.getID()] = HS[u.getID()]
            else:
                vl,vr = v.getLeft().getID(),v.getRight().getID()
                HS[v.getID()] = updateHS(HS[vl],HS[vr])
                #print(str(v.getID())+' '+str(HS[v.getID()])+' '+str(vl)+' '+str(HS[vl])+' '+str(vr)+' '+str(HS[vr]))
            nodesQueue.append(v)
    return(HS[t.getID()])
